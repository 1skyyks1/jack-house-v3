...
export const PlayerPerformancePoster = forwardRef<SVGSVGElement, PlayerPerformancePosterProps>(function PlayerPerformancePoster({ profile, snapshotDate, tournament }, ref) {
  ...
  const bestGames = [...profile.entries].sort((a, b) => b.jpp - a.jpp || b.score - a.score).slice(0, 3)
  ...
  return (
    <svg aria-label={`${playerName} · ${tournament.name}`} className="block h-auto w-full" ref={ref} role="img" viewBox="0 0 340 500" xmlns="http://www.w3.org/2000/svg">
      ...
      <text fill={SLATE} fontFamily={FONT_DISPLAY} fontSize="11" fontWeight="700" textAnchor="end" x="316" y="32">{formatPosterDate(snapshotDate)}</text>
      ...
      <text fill={SLATE} fontFamily={FONT_NUMERIC} fontSize="25" fontWeight="700" textAnchor="end" x="316" y="95">{profile.rank ? `#${profile.rank}` : "—"}</text>
      ...
      {stageRows.map(({ key, label, stage }, index) => {
        ...
        const y = 155 + row * 56
        const winRatio = stage?.games ? stage.wins / stage.games : 0
        const scoreWidth = winRatio * 136
        return (
          <g key={key}>
      ...
      {bestGames.map((entry, index) => {
        ...
        return (
          <g key={`${entry.game_id}-${entry.side}`}>
            ...
            <text fontFamily={FONT_NUMERIC} fontSize="12" fontWeight="700" textAnchor="end" x="316" y={y}>
              <tspan fill={entry.rating_delta >= 0 ? ROSE : INK}>{formatDelta(entry.rating_delta)}</tspan>
              <tspan dx="7" fill={SLATE}>{entry.rank ? `#${entry.rank}` : "—"}</tspan>
            </text>
      ...
      <text fill="#666663" fontFamily={FONT_NUMERIC} fontSize="9" fontWeight="600" textAnchor="end" x="316" y="483">{profile.entries.length} GAMES / TPR {Math.round(profile.tournamentRating)}</text>
    </svg>
  )
})
