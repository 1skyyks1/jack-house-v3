import { CaretLeft, CaretRight, ChartBar, Medal, Trophy } from "@phosphor-icons/react"
...
import { AppAlert, getErrorMessage, SectionTitle } from "@/shared/components"
...
export function PackLeaderboard({ beatmapId, packId, title, version }: PackLeaderboardProps) {
  ...
  const leaderboardQuery = usePackLeaderboardQuery({ beatmapId, packId, page, pageSize: PAGE_SIZE })
  ...
  return (
    <section className="border-t p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="inline-flex items-center gap-2">
            <SectionTitle>{t("pack.leaderboard.title")}</SectionTitle>
            <Trophy className="-translate-y-px size-5 shrink-0 text-amber-500" weight="fill" />
          </div>
      ...
      {leaderboard.personal ? <PersonalScore entry={leaderboard.personal} /> : null}
      ...
      {leaderboard.data.length > 0 ? (
        <div className="mt-5">
          ...
          <div className="divide-y">
            {leaderboard.data.map((entry) => <LeaderboardRow entry={entry} key={entry.user_id} />)}
          </div>
        </div>
      ) : (
      ...
      {leaderboard.totalPages > 1 ? (
        <div className="mt-4 flex items-center justify-end gap-2">
          <Button disabled={page <= 1} onClick={() => setPage((value) => Math.max(value - 1, 1))} size="icon-sm" type="button" variant="outline">
            <CaretLeft className="size-4" weight="bold" />
          </Button>
          <span className="text-sm text-muted-foreground">{t("common.pageStatus", { page, total: leaderboard.totalPages })}</span>
          <Button disabled={page >= leaderboard.totalPages} onClick={() => setPage((value) => value + 1)} size="icon-sm" type="button" variant="outline">
...
function PersonalScore({ entry }: { entry: PackLeaderboardEntry }) {
  const { t } = useTranslation()
  return (
    <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 py-2 text-sm">
      <div className="inline-flex items-center gap-2 font-medium">
        <Medal className="size-5 text-primary" weight="fill" />
        {t("pack.leaderboard.personal")}
      </div>
      <span className="text-sm text-muted-foreground">{t("pack.leaderboard.rank")}: <strong className="text-foreground">#{entry.rank}</strong></span>
      <span className="text-muted-foreground">{t("pack.leaderboard.score")}: <strong className="text-foreground">{formatScore(entry.score)}</strong></span>
      <ScoreDate entry={entry} />
    </div>
  )
}

function LeaderboardRow({ entry }: { entry: PackLeaderboardEntry }) {
  return (
    <div className="grid grid-cols-[1.5rem_minmax(0,1fr)_3.75rem_2.75rem_2.5rem_3.5rem] items-center gap-0.5 px-0.5 py-2 lg:grid-cols-[2.25rem_minmax(7rem,1fr)_repeat(6,3.25rem)_6rem_4.5rem_3rem_6rem_8.5rem] lg:gap-2 lg:px-2">
      ...
      <Link className="flex min-w-0 items-center gap-2 text-xs hover:opacity-80 sm:text-sm" to={`/user/${entry.user_id}`}>
        ...
        <span className="truncate font-medium">{entry.user.user_name}</span>
      </Link>
